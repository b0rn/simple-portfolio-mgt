...
class TestPortfolioMgtUseCase:
    ...
    async def test_create_portfolio(self, mock_db_dataservice : DbDataService):
        portfolio = Portfolio(id=0, owner_id=uuid4(),name="foo", created_at=datetime.now())
    ...
    async def test_get_portfolio(self, mock_db_dataservice : DbDataService):
        ...
        assert res == portfolio
        mock_db_dataservice.get_portfolio.assert_awaited_once_with(owner_id, portfolio.id)
    ...
    async def test_update_portfolio(self, mock_db_dataservice : DbDataService):
        ...
        res = await uc.update_portfolio(owner_id=owner_id, portfolio_id=portfolio.id, payload=payload)
        
        assert res == portfolio
        mock_db_dataservice.update_portfolio.assert_awaited_once_with(owner_id=owner_id, portfolio_id=portfolio.id, payload=payload)
    ...
    async def test_list_portfolios_paginated(self, mock_db_dataservice : DbDataService):
        ...
        page_res = PaginationResponse(total_items=2, total_pages=1, current_page=1, items_per_page=20)
        mock_db_dataservice.list_portfolios_paginated = AsyncMock()
        mock_db_dataservice.list_portfolios_paginated.return_value = portfolios, page_res
        uc = self.__get_uc(mock_db_dataservice=mock_db_dataservice)
        
        owner_id = "my-id"
        page_req = PaginationRequest(items_per_page=5, page=8)
        res_portfolios, res_page_res = await uc.list_portfolios_paginated(owner_id=owner_id, pagination_request=page_req)
        ...
        mock_db_dataservice.list_portfolios_paginated.assert_awaited_once_with(owner_id, page_req)
        
    async def test_compute_portfolio_valuation(self, mock_db_dataservice : DbDataService):
        prices = (self.__get_uc(mock_db_dataservice)).get_assets_prices()
        symbols = [k for k in prices.keys()]
        assert len(symbols) > 1
        assets = [
            Asset(id=0,portfolio_id=0, symbol=symbols[0], quantity=5, created_at=datetime.now()),
            Asset(id=5,portfolio_id=0, symbol=symbols[len(symbols) - 1], quantity=5, created_at=datetime.now()),
            Asset(id=8,portfolio_id=0, symbol="UNKNOWN", quantity=544, created_at=datetime.now())
        ]
        ...
        assert res.unknown_symbols == [a.symbol for a in assets if a.symbol not in symbols]
        total_value = 0
        for i in range(len(res.lines)):
            ...
            price = next((v for k,v in prices.items() if res.lines[i].symbol == k), None)
            if price is None:
    ...
    async def test_create_asset(self, mock_db_dataservice : DbDataService):
        asset = Asset(id=0, portfolio_id=5, symbol="BTC", quantity=8, created_at=datetime.now())
    ...
    async def test_list_assets_paginated(self, mock_db_dataservice : DbDataService):
        assets = [
            Asset(id=0, portfolio_id=0, symbol="BTC", quantity=4, created_at=datetime.now()),
            Asset(id=1, portfolio_id=0, symbol="ETH", quantity=0.07, created_at=datetime.now())
        ]
        page_res = PaginationResponse(total_items=8, total_pages=4, current_page=2, items_per_page=7)
